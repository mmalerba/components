...
// TODO(mmalerba): Remove this once formGroup is zoneless compatible.
@Directive({
  selector: '[formGroup]',
  standalone: true,
})
class FixFormGroup implements OnInit {
  private readonly _changeDetectorRef = inject(ChangeDetectorRef);
  private readonly _formGroup = inject(FormGroupDirective);
  private readonly _destroyRef = inject(DestroyRef);

  ngOnInit() {
    this._formGroup.control.events.pipe(takeUntilDestroyed(this._destroyRef)).subscribe(() => {
...
@Component({
  ...
  `,
  imports: [ReactiveFormsModule, MatStepperModule, FixFormGroup],
})
