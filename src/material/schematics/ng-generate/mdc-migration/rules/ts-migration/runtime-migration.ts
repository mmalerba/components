...
export class RuntimeCodeMigration extends Migration<ComponentMigrator[], SchematicContext> {
  ...
  private _migrateSourceFileReferences(sourceFile: ts.SourceFile) {
    const {moduleSpecifiers, importSpecifiers} = this._findImportsToMigrate(sourceFile);

    [
      ...this._renameModuleSpecifiers(moduleSpecifiers),
      ...this._renameReferences(sourceFile, importSpecifiers),
    ]
  ...
  private _findImportsToMigrate(sourceFile: ts.SourceFile) {
    const importSpecifiers = new Map<ts.ImportSpecifier, string>();
    ...
    for (const statement of sourceFile.statements) {
      ...
      statement.importClause.namedBindings.elements.forEach(element => {
        const newName = this._removeLegacy((element.propertyName || element.name).text);

        if (newName) {
          importSpecifiers.set(element, newName);
        }
      });
    ...
    return {importSpecifiers, moduleSpecifiers};
  }

  private _renameReferences(
    sourceFile: ts.SourceFile,
    importSpecifiers: Map<ts.ImportSpecifier, string>,
  ): Replacement[] {
    ...
    const walk = (node: ts.Node) => {
      ...
      if (ts.isIdentifier(node)) {
        for (const [specifier, newName] of importSpecifiers.entries()) {
          ...
          if (!specifier.propertyName && this._isReferenceToImport(node, specifier)) {
            replacements.push([node, newName]);
          }
        }
      }
    ...
    importSpecifiers.forEach((newName, specifier) => {
  ...
  private _removeLegacy(name: string): string | null {
    const legacyRegex = /legacy_|legacy-|legacy/i;
